...
const TemplateDetails = () => {
  ...
  const [selectedAssessmentType, setSelectedAssessmentType] = useState("Enrollment Assessment");
  ...
  const [selectedDefaultAssessment, setSelectedDefaultAssessment] = useState(false);
    ...
    CrudService.getSingle("Template", id).then(async (res) => {
        if (!res.data) return;
            ...
            await CrudService.search("DefaultAssessment", 1000, 1, {}).then((res) => {
                ...
                const data = defaultAssessment.filter(item => !templateData.assessments.includes(item._id)).map(item => ({ ...item }));
  ...
  const fetchAssessmentData = async (
    ...
    const response = await CrudService.search("DefaultAssessment", limit, page, data);
  ...
  const handleModalOk = async () => {
      if (selectedDefaultAssessment) {
          const selectedAssessment = defaultAssessmentData.find(item => item._id === selectedDefaultAssessment);
            ...
            <div>
              ...
              templateData?.impactThemDetails.length > 0 && (
                  <p className={"py-2"}>
                    ...
                    {templateData?.impactThemDetails.map(
                        (item, index) => (
                            <>
                              <span key={item._id}>{item.Name}</span>
                              {index !==
                              templateData.impactThemDetails.length - 1
                                  ? ", "
                                  : ""}
                            </>
                        )
              ...
              {templateData?.strategicGoalDetails && (
                  <p className={"py-2"}>
                    <strong>Strategic Goal</strong>: {templateData.strategicGoalDetails.Name}
                  </p>
              )}
              {templateData?.deliveryModelDetails && (
                  <p className={"py-2"}>
                    <strong>Delivery Model</strong>: {templateData.deliveryModelDetails.Name}
                  </p>
              )}
              {templateData?.productDetails && (
                  <p className={"py-2"}>
                    <strong>Products And Services</strong>: {templateData.productDetails.Name}
                  </p>
              )}
              ...
              {templateData?.startDate && (
                  <p className={"py-2"}>
                    ...
                    {moment(templateData.startDate).format(
                        "LLLL"
                    )}
                  </p>
              )}
              {templateData?.endDate && (
                  <p className={"py-2"}>
                    ...
                    {moment(templateData.endDate).format(
                        "LLLL"
                    )}
                  </p>
              )}
    ...
    },
    {
      ...
      children: (
          <>
            ...
            <Modal  width={1150} open={KPIModal} onCancel={() => setKPIModal(false)}>
              <div>
                <input
                    type="text"
                    name="search"
          ...
          <>
            <div className="flex mb-3">
              ...
              <Button
                  ...
                  type="primary"
                  onClick={async () => {
                      navigate(`/dashboard/templatepre?templateId=${templateData._id}&assessmentType=${assessmentType}`);
                  }}
              >
              ...
              >
                ...
                <Modal width={500} open={isModalOpen} onOk={handleModalOk} onCancel={handleModalCancel}>
                    ...
                    >
                        {defaultAssessmentData && defaultAssessmentData.map((step) => (
            ...
            <div className={"relative assessments-tabs"}>
              ...
              {assessmentData?.map?.((assessment) => (
                  ...
                  >
                    ...
                    <div className="flex-1 px-4">{assessment.description || "-"}</div>
                    <div className="flex-1 px-4">
                    ...
                    <div className="flex-1 px-4">
                      <Space>
                        <Button
                            ...
                            onClick={() => {
                              navigate(
                                  `/dashboard/templatepre?id=${assessment._id}`
                              );
                            }}
                        >
                          Edit assessment
                        ...
                        <Popconfirm
                            ...
                            onConfirm={async () => {
                                const assessments = templateData.assessments.filter(id => id !== assessment._id);
                                const mainDefaultAssessment = await CrudService.search("DefaultAssessment", 1, 1, {sort: {createdAt: 1}});
                                if (mainDefaultAssessment.data.items[0]._id === assessment._id) {
                                    const id = searchParams.get("id");
                                    if (!id) return;
                                    await CrudService.update("Template", id, {
