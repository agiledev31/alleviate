...
const ProgramDetails = () => {
  ...
  const [qualificationActiveTab, setQualificationActiveTab] = useState('qualified');
  const [qualificationModal, setQualificationModal] = useState(false);
  const [questionSets, setQuestionSets] = useState([{ id: 1, fieldName: '', fieldAnswer: '', qualifyOption: 'include', operator: '' }]);
  ...
  useEffect(() => {
    ...
    CrudService.getSingle("Program", id).then((res) => {
      ...
      if (res.data.hasOwnProperty('formAccessibility') && res.data.formAccessibility === 'public') {
    ...
    },
    {
      ...
      render: (record) => (
        <Space size={[12, 10]} wrap>
          ...
          <Modal width={800} height={300} open={submissionModal} onOk={() => setSubmissionModal(false)} onCancel={() => {setSubmissionModal(false); setSelectedSubmissionData(null)}}>
            <>
          ...
          <Popconfirm
              ...
              onConfirm={async () => {
                ...
                const data = submittedProgramData.filter?.((a) => a._id !== record.data._id)
          ...
          {(record.data.formData.hasOwnProperty('submittedData') && activeTab === 'waiting') && (
              <>
                {record.data.status === 'not-approve' && (
                    <>
                      <Button type={"primary"}
                              onClick={async () => {
                                const updatedSubmission = await CrudService.update("ProgramSubmission", record.data._id, {status: 'approve'});
                                if (updatedSubmission) {
                                  const updatedSubmissionDataList = submittedProgramData.map(item => {
                      ...
                      <Popconfirm
                          ...
                          onConfirm={async () => {
                            const updatedSubmission = await CrudService.update("ProgramSubmission", record.data._id, {status: 'reject'});
                            if (updatedSubmission) {
                              const updatedSubmissionDataList = submittedProgramData.map(item => {
                                if (item._id === record.data._id) {
                            ...
                            onClick={async () => {
                              ...
                              if (updatedSubmission) {
                                if (qualificationActiveTab === 'qualified') {
                                  const updatedDataList = qualificationData.map(item => {
                                    if (item._id === record.data._id) {
                                      return { ...item, status: 'approve' };
                                    }
                                    return item;
                                  ...
                                  const updatedDataList = unQualificationData.map(item => {
                                    if (item._id === record.data._id) {
                                      return { ...item, status: 'approve' };
                                    }
                                    return item;
  ...
  const handleAddQuestionSet = () => {
    ...
    setQuestionSets([...questionSets, { id: newId, fieldName: '', fieldAnswer: '', qualifyOption: 'include', operator: '' }]);
  };
  ...
  const handleRemoveQuestionSet = (id, index) => {
    ...
    if (lastSetIndex === index) {
      ...
      setQuestionSets((prevSets) =>
          prevSets.map((set) => (set.id === prevId ? { ...set, operator: '' } : set))
      )
    }
  };
  ...
  const handleQualifyOptionChange = (value, id) => {
    setQuestionSets((prevSets) =>
        prevSets.map((set) => (set.id === id ? { ...set, qualifyOption: value } : set))
    );
  ...
  const handleFieldAnswerChange = (value, id) => {
    setQuestionSets((prevSets) =>
        prevSets.map((set) => (set.id === id ? { ...set, fieldAnswer: value } : set))
    );
  ...
  const handleQualifications = () => {
    ...
    setQuestionSets([{ id: 1, fieldName: '', fieldAnswer: '', qualifyOption: 'include', operator: '' }]);

    const fields = programData.form.flatMap(item => item.form.map(data => ({
  ...
  const handleSelectProgramField = (value, id) => {
    ...
    setQuestionSets((prevSets) =>
        prevSets.map((set) => (set.id === id ? { ...set, fieldName: value, fieldAnswer: '' } : set))
    );
  ...
  const handleQualificationSearch = () => {
    ...
    CrudService.search("ProgramSubmission", 1000, PAGE_NUMBER, {
      ...
      const qualifiedOriginalData = submittedProgramData.filter(item => qualifiedDataIds.includes(item._id) && item.formData.submittedData);
      const unQualifiedOriginalData = submittedProgramData.filter(item => !qualifiedDataIds.includes(item._id) && item.formData.submittedData && item.status === 'not-approve');
    ...
    <>
      <div style={{ height: "80vh" }}>
        ...
        <div className="mt-3">
          <Button
              type="primary"
              onClick={() => {setDataSummeryModal(true)}}
          >
            View Data Summery
          </Button>
          <Modal width={1000} open={dataSummeryModal} onOk={() => setDataSummeryModal(false)} onCancel={() => setDataSummeryModal(false)}>
            <StatsDashboard type={"assessment"} />
          </Modal>
        </div>
        ...
        <div className="d-flex items-center pb-4">
          <Space size={24}>
            ...
            <Modal width={1500} open={qualificationModal} onOk={handleQualificationModalClose} onCancel={handleQualificationModalClose}>
              ...
              <div className={"qualification-modal-spinner"}>
                ...
                {questionSets.map((questionSet, index) => (
                    <div className={'qualification-modal-form'} key={questionSet.id}>
                      <div className={'qualification-select'}>
                        <h4>Select Question</h4>
                        <Select
                            options={programFields}
                            value={questionSet.fieldName}
                            onChange={(value) => handleSelectProgramField(value, questionSet.id)}
                        />
                      </div>
                      ...
                      <div className={'qualification-options'}>
                        ...
                        <Radio.Group
                            onChange={(e) => handleQualifyOptionChange(e.target.value, questionSet.id)}
                            value={questionSet.qualifyOption}
                        >
                          <Radio defaultChecked={true} value="include"> Include </Radio>
                      ...
                      <div className={'qualification-select'}>
                        ...
                        <Select
                            ...
                            onChange={(value) => handleFieldAnswerChange(value, questionSet.id)}
                        />
                      </div>
                      ...
                      <div className={'qualification-options'}>
                        ...
                        <Radio.Group
                            onChange={(e) => onQualificationOperatorsChange(e.target.value, questionSet.id)}
                      ...
                      {questionSet.id > 1 && (
                          <Button type="primary" onClick={() => handleRemoveQuestionSet(questionSet.id, index)}>
                            Remove Criteria Set
                          </Button>
                      )}
                    </div>
              ...
              <Divider />
              <div>
                <Tabs activeKey={qualificationActiveTab} onChange={(key) => setQualificationActiveTab(key)}>
                  <TabPane key="qualified" tab="Qualified">
                    {qualificationData.length > 0 ? (
                        <Table
                            dataSource={qualificationData
                                .map((item, index) => ({
                                  ...
                                  submissionType:
                                      item.formData.hasOwnProperty('submittedData') && 'Online' ||
                                      item.formData.hasOwnProperty('fileData') && 'Imported Data',
                                  status: (() => {
                        ...
                        <div><strong>No Qualified Data Found</strong></div>
                    )}
                  </TabPane>
                  <TabPane key="unQualified" tab="UnQualified">
                    {unQualificationData.length > 0 ? (
                        <Table
                            dataSource={unQualificationData
                                .map((item, index) => ({
                                  key: index,
                                  userName: item.user_id?.firstName + ' ' + item.user_id?.lastName,
                                  userEmail: item.user_id?.email,
                        ...
                        <div><strong> No Un Qualified Data Found </strong></div>
                    )}
                  </TabPane>
                </Tabs>
              </div>
        ...
        <Tabs activeKey={activeTab} onChange={(key) => setActiveTab(key)}>
          <TabPane tab="All" key="all">
            {submittedProgramData.length > 0 ? (
                    ...
                    dataSource={submittedProgramData
                        .map((item, index) => ({
                          ...
                          submissionType:
                              ...
                              item.formData.hasOwnProperty('fileData') && 'Imported Data',
                          status: (() => {
                            if (item.formData.isFinishClicked && item.formData.hasOwnProperty('fileData')) {
                              return 'Approved';
                ...
                <div><strong>No Submission Data Found</strong></div>
            )}
          </TabPane>
          <TabPane tab="Approved" key="approved">
            {submittedProgramData.length > 0 ? (
                ...
                <div><strong> No Approved Submission Data Found </strong></div>
            )}
          </TabPane>
          <TabPane tab="Waiting List" key="waiting">
            {submittedProgramData.length > 0 ? (
                ...
                <div><strong> No Waiting Submission Data Found </strong></div>
            )}
          </TabPane>
          <TabPane tab="Rejected" key="reject">
            {submittedProgramData.length > 0 ? (
                ...
                <div><strong> No Rejected Submission Data Found </strong></div>
            )}
          </TabPane>
        </Tabs>
      </div>
