...
const SuiteDetail = () => {
  ...
  const fetchAssessmentData = async (limit, page) => {
    setShowLoader(true);
    const data = {
    ...
    if (hasInvitedEmails) {
      ...
      const updatedResponse = await CrudService.search(
        "Program",
        limit,
        page,
        data
  ...
  const updateAssessmentData = async (
    ...
    const assessmentData = await fetchAssessmentData(limit, page);
    setShowLoader(false);
  ...
  const fetchSubmissions = async () => {
    const response = await CrudService.search("ProgramSubmission", 1000, 1, {
      filters: {
        programId: assessmentData.map((item) => item._id),
        user_id: user._id,
      },
    });
      ...
      children: (
        <>
          <Tabs
            ...
            onChange={(key) => setActiveSubmissionTab(key)}
          >
            <TabPane tab="All" key="all">
              {submissionData.length > 0 ? (
                <Table
                  dataSource={submissionData.map((item, index) => ({
                    key: index,
